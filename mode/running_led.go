...
func (self *ModeRunningLed) postSetParameter() {
	...
	self.positionDegStepSize = 360.0 / (float64(self.parameter.RoundTimeMs) / 1000.0 /*s*/) * (float64(FPS) / 1000.0 /*s*/ / 1000.0 /*ms*/ / 1000.0 /*us*/)
	self.darkenStepSize = (1 / self.parameter.FadePct) / (float64(self.parameter.RoundTimeMs) / 1000.0 /*s*/) * (float64(FPS) / 1000.0 /*s*/ / 1000.0 /*ms*/ / 1000.0 /*us*/)
	self.lightenStepSize = 2 * self.parameter.Brightness * (float64(self.display.GetRowLedCount())) / (float64(self.parameter.RoundTimeMs) / 1000.0 /*s*/) * (float64(FPS) / 1000.0 /*s*/ / 1000.0 /*ms*/ / 1000.0 /*us*/)
}
